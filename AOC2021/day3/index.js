...
// numbers (called the gamma rate and the epsilon rate). The power consumption can then be found by
...
let inputAsString = buildString();
let inputAsArray = buildArray(inputAsString);

// Utility method to flip bits in JS binary string
const flipbits = (str) => {
  let flipped = str
    .split("")
...
// Part One
const binaryDiagnostic = (binaryReport) => {
  let digitPlace = 0;
  let output = "";
  ...
  while (digitPlace < 12) {
    for (let i = 0; i < binaryReport.length; i++) {
      binaryReport[i][digitPlace] == 0
        ? (countMap.off += 1)
        : (countMap.on += 1);
    }

    countMap.on > countMap.off ? (output += 1) : (output += 0);
  ...
  return parseInt(output, 2) * parseInt(flipbits(output), 2);
};

console.log(binaryDiagnostic(inputAsArray));
