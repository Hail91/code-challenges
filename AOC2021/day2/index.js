...
const dive = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    checkMap(input[i], output);
  }

  return output.horizontalProgress * output.depth;
};

const checkMap = (value, currentOutput) => {
  let instruction = value.split(" ");

  switch (instruction[0]) {
    case "forward":
      currentOutput.horizontalProgress += Number(instruction[1]);
      break;
    case "down":
      currentOutput.depth += Number(instruction[1]);
      break;
    case "up":
      currentOutput.depth -= Number(instruction[1]);
      break;
...
console.log(dive(problemInputAsArray));
...
const diveTwo = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    calculateData(input[i], courseData);
  }

  return courseData.horizontalProgress * courseData.depth;
};

const calculateData = (value, data) => {
  let instruction = value.split(" ");

  switch (instruction[0]) {
    case "forward":
      data.horizontalProgress += Number(instruction[1]);
      data.depth += data.aim * Number(instruction[1]);
      break;
    case "down":
      data.aim += Number(instruction[1]);
      break;
    case "up":
      data.aim -= Number(instruction[1]);
      break;
...
console.log(diveTwo(problemInputAsArray));
